...
qpsk_signal = partial(
    tx_qpsk, sample_rate=SAMPLE_RATE, symbol_rate=SYMBOL_RATE, carrier_freq=CARRIER_FREQ
)
bfsk_signal = partial(
    tx_bfsk,
    sample_rate=SAMPLE_RATE,
...
harness: List[ModulationTestResults] = [
    ...
    ModulationTestResults(name="QPSK", tx_func=qpsk_signal),
...
def __parse_args() -> Namespace:
    ...
    parser.add_argument("-a", "--num-attempts", type=int, default=150)
    parser.add_argument("-s", "--num-snrs", type=int, default=100)
...
if __name__ == "__main__":
    ...
    snrs: np.array = undb(np.linspace(-25, -6, __args.num_snrs))
    progress = tqdm(total=len(snrs) * len(harness))
    results: List[ModulationTestResults] = [
        __run(
            modulation,
            snrs,
            num_attempts=__args.num_attempts,
            progress=progress,
        )
