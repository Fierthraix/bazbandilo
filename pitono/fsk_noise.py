...
class BitErrorTestResults:
    ...
    def get_n0s(self, num_bits: int = 9056) -> np.array:
        ...
        return np.nan_to_num(np.sqrt(eb / (2 * self.samples_per_symbol * self.snrs)))
...
if __name__ == "__main__":
    ...
    # fsk_consts: List[int] = list(range(32, 65, 2))
    fsk_consts: List[int] = list(range(38, 42))
    ...
    for scheme in comms_schemes:
        ...
        #     continue
        ax.plot(db(scheme.snrs), scheme.bers, linestyle='dashed', label=f"{scheme.name} BER")
